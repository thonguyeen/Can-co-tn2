...
## Giai đoạn 6: Issue Documentation (Knowledge Capture)

Sau khi sự cố/vấn đề đã được xử lý xong, bạn BẮT BUỘC tạo tài liệu issue để lưu vết và phục vụ tra cứu sau này.

### 6.1. Mục tiêu

- Lưu lại lỗi đã gặp
- Ghi nhận nguyên nhân gốc
- Ghi lại cách đã xử lý
- Tạo tài liệu tham khảo nếu lỗi tương tự lặp lại

### 6.2. Vị trí lưu

Tất cả issue phải được lưu trong:

`docs/issues/`

Nếu thư mục chưa tồn tại, AI phải tạo mới.

### 6.3. Quy tắc bắt buộc

- Không được kết thúc `/debug` nếu chưa tạo issue document.
- Mỗi sự cố/vấn đề phải có 1 file issue riêng.
- Issue phải được viết sau khi đã xác định nguyên nhân và cách xử lý.
- Không ghi issue mơ hồ kiểu “đã sửa lỗi”.
- Phải ghi rõ bằng chứng, nguyên nhân, cách sửa và cách kiểm tra lại.

### 6.4. Quy tắc đặt tên file

Format:

`YYYY-MM-DD-[type]-[short-issue-title].md`

Ví dụ:

`2026-04-27-n8n-schedule-random-fixed-minute.md`
...
[Mô tả ngắn gọn sự cố trong 1-3 câu]

## Hiện tượng

[Người dùng thấy gì? Lỗi xảy ra ở đâu? Khi nào?]
...
[Giải thích nguyên nhân thật sự bằng ngôn ngữ dễ hiểu]
...
[Các bước xác nhận lỗi đã được sửa]
...
### 6.6. Checklist trước khi kết thúc /debug
Trước khi báo “đã xong”, bạn phải tự kiểm tra:

- [ ] Đã xác định nguyên nhân gốc
- [ ] Đã sửa đúng vị trí
- [ ] Đã kiểm tra lại
- [ ] Đã xóa debug log tạm thời
- [ ] Đã cập nhật `session.json` nếu có
- [ ] Đã tạo issue trong `docs/issues/`
- [ ] Đã báo user cách kiểm tra lại

### 6.7. Câu kết thúc chuẩn:
Sau khi tạo issue xong, AI báo:
Đã sửa xong và đã lưu tài liệu issue tại docs/issues/[tên-file].md để sau này tra cứu lại.
