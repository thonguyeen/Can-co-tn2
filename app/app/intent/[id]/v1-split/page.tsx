...
const LeafletIsoMap = dynamic(() => import('@/components/map/LeafletIsoMap').then(mod => mod.LeafletIsoMap), {
    ssr: false,
    loading: () => <div className="w-full h-full bg-slate-100 flex items-center justify-center"><div className="w-6 h-6 border-2 border-indigo-500 border-t-transparent animate-spin rounded-full" /></div>
});
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-slate-50 px-4">
                <div className="text-center">
                    <Home className="w-12 h-12 text-indigo-300 mx-auto mb-4" />
                    <p className="text-slate-500 font-medium mb-6">Không tìm thấy bài đăng</p>
                    <button
                        onClick={() => router.back()}
                        className="px-6 py-2.5 bg-indigo-600 text-white rounded-full text-sm font-bold cursor-pointer"
                    >Quay lại</button>
                </div>
            </div>
        );
    ...
    return (
        <div className="flex h-[calc(100vh-3.5rem)] overflow-hidden bg-slate-50">
                ...
                <div className="hidden lg:flex w-1/2 flex-col shrink-0 relative">
                    ...
                    {/* ── Trust badge (top-right của image slider) ── */}
                    <div className="absolute top-3 right-3 z-20 flex items-center gap-1.5 px-3 py-1.5 bg-white/95 backdrop-blur-sm rounded-full text-xs font-bold text-emerald-700 border border-emerald-200 shadow-sm">
                    ...
                    <div className="h-1/2 relative border-t border-slate-200">
                ...
                <div className="flex-1 flex flex-col overflow-hidden bg-white">
                    ...
                    {/* ── Mobile hero (no map on mobile) — back button sudah di header ── */}
                        ...
                        <div className="px-6 lg:px-8 pt-6 pb-24">
                            ...
                            {!isEditing && (
                                <>
                                    ...
                                    {/* ── Tiện ích xung quanh (ORS POIs) ── */}
                                    <div className="border-t border-slate-100 mb-6" />
                                    <POILayer
                                        lat={intent.lat ?? 10.7769}
                                        lng={intent.lng ?? 106.7009}
                                        radiusMeters={1200}
                                        className="mb-6"
                                    />
