...
export function ComposeIntent({ mode = 'demo', onSubmit, onIntentCreated, editIntent, onEditComplete, onCancelEdit }: ComposeIntentProps) {
      ...
      <>
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <span className="text-sm font-medium text-slate-500">Bạn đang CẦN tìm gì, hay CÓ gì muốn bán?</span>
          </div>
        </button>
        {toast && <Toast {...toast} />}
      </>
  ...
  return (
    <>
      <div className="bg-white rounded-3xl border border-slate-200 shadow-[0_4px_20px_rgba(0,0,0,0.06)] overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
          <span className="text-sm font-bold text-slate-800 uppercase tracking-wider">{isEditMode ? 'Chỉnh sửa bài viết' : 'Đăng nhu cầu'}</span>
          <button
        ...
        <div className="p-5 space-y-4">
          ...
          <textarea
            ...
            rows={4}
            className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 text-slate-700 text-sm leading-relaxed outline-none focus:bg-white focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/10 transition-all resize-none placeholder:text-slate-400"
            autoFocus
          />
          ...
          {(showTags && tags.length > 0) && (
            <div className="p-3.5 bg-indigo-50/50 border border-indigo-100 rounded-2xl">
              <div className="flex items-center gap-1.5 mb-2.5">
                ...
                {isParsing && <Loader2 className="w-3.5 h-3.5 animate-spin text-indigo-400 ml-1" />}
              </div>
              <div className="flex flex-wrap gap-1.5">
                {tags.map((tag, i) => (
                  <span
                    key={i}
                    className="inline-flex items-center gap-1 text-[11px] font-semibold px-2.5 py-1 rounded-full bg-white text-indigo-700 border border-indigo-200/60 shadow-sm"
                  >
                    {tag.icon} {tag.label}
                  </span>
                ))}
          ...
          {isParsing && !showTags && (
            <div className="flex items-center gap-2 text-xs font-medium text-slate-500 px-1 py-0.5">
              <Loader2 className="w-3.5 h-3.5 animate-spin text-indigo-500" />
              <span>AI đang phân tích ý định của bạn...</span>
            </div>
          )}
          ...
          <div className="flex items-center justify-between mb-1 px-1">
            <span className={cn(
              ...
              text.trim().length >= 10 ? 'text-emerald-500' : 'text-slate-400',
            )}>
              {text.trim().length}/10 ký tự {text.trim().length < 10 ? '(tối thiểu 10)' : '✓ Đủ điều kiện'}
            </span>
          </div>
          <button
            ...
            className={cn(
              'relative w-full py-3.5 rounded-2xl text-[15px] font-bold text-white transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed shadow-md',
              type === 'CAN'
                ? 'bg-red-500 hover:bg-red-600 hover:shadow-red-500/25'
                : 'bg-emerald-500 hover:bg-emerald-600 hover:shadow-emerald-500/25',
              isSubmitting ? 'shadow-none' : 'hover:-translate-y-0.5'
            )}
          >
            {isSubmitting ? (
              ...
              'Đăng Nhu Cầu Tìm Kiếm'
            ) : (
              'Đăng Nhu Cầu Mua Bán'
            )}
          </button>
        </div>
      </div>
